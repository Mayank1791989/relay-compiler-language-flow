...
import RelayMaskTransform from 'relay-compiler/lib/RelayMaskTransform';
...
import {
  ...
  unionTypeAnnotation,
} from './RelayFlowBabelFactories';
import {
  transformScalarType,
  transformInputType,
} from './RelayFlowTypeTransformers';
import {
...
export type State = {|
  ...
  // patch original javascript language
  +importCustomScalars: Set<string>,
  +customScalarsModule: ?string,
|};
...
function makeProp(
  ...
  if (schemaName === '__typename' && concreteType) {
    // eslint-disable-next-line no-param-reassign
    _value = t.stringLiteralTypeAnnotation(concreteType);
  }

  const typeProperty = readOnlyObjectTypeProperty(key, _value);
  if (conditional) {
...
function createVisitor(options: TypeGeneratorOptions) {
  const state = {
    ...
    importCustomScalars: new Set(),
    // HACK: reading module value using special key inside customScalars
    customScalarsModule: options.customScalars.$module$,
  };
...
function getScalarImports(state: State) {
  const imports = [];
  if (state.importCustomScalars.size > 0) {
    imports.push(
      importTypes(
        Array.from(state.importCustomScalars),
        nullthrows(state.customScalarsModule),
      ),
    );
  }
  return imports;
